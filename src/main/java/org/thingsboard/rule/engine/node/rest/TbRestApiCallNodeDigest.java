...
@RuleNode(
        ...
        name = "rest api call digest",
        configClazz = TbRestApiCallNodeConfigurationDigest.class,
        version = 1,
        nodeDescription = "Invoke REST API calls to external REST server using digest auth",
        nodeDetails = "Will invoke REST API call <code>GET | POST | PUT | DELETE</code> to external REST server using digest auth. " +
...
public class TbRestApiCallNodeDigest extends TbAbstractExternalNode {

    static final String PARSE_TO_PLAIN_TEXT = "parseToPlainText";
    static final String TRIM_DOUBLE_QUOTES = "trimDoubleQuotes";
    protected TbHttpClientDigest httpClient;

    @Override
    public void init(TbContext ctx, TbNodeConfiguration configuration) throws TbNodeException {
        super.init(ctx);
        TbRestApiCallNodeConfigurationDigest config = TbNodeUtils.convert(configuration, TbRestApiCallNodeConfigurationDigest.class);
        httpClient = new TbHttpClientDigest(config, ctx.getSharedEventLoop());
    }
