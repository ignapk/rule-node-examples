...
public class TbHttpClientDigest {
        ...
        future.addCallback(new ListenableFutureCallback<ResponseEntity<String>>() {
            ...
            public void onFailure(Throwable throwable) {
                if (throwable instanceof HttpStatusCodeException &&
                    ...
                    handleDigestChallenge(ctx, msg, throwable);
                } else {
    ...
    private void handleDigestChallenge(TbContext ctx, TbMsg msg,
        ...
        if (wwwAuthenticateHeader != null && wwwAuthenticateHeader.startsWith("Digest")) {
            ...
            retryWithDigestAuth(ctx, msg, nonce, realm, opaque);
        } else {
            TbMsg next = processException(ctx, msg, throwable);
            ctx.tellNext(next, TbRelationTypes.FAILURE);
        }
    ...
    private void retryWithDigestAuth(TbContext ctx, TbMsg msg,
        ...
        URI uri = buildEncodedUri(endpointUrl);
        HttpHeaders headers = prepareHeaders(msg);
        headers.add("Authorization", buildDigestAuthHeader(nonce, realm, opaque, uri.getPath()));
        HttpMethod method = HttpMethod.valueOf(config.getRequestMethod());
        HttpEntity<String> entity;
        if (HttpMethod.GET.equals(method) || HttpMethod.HEAD.equals(method) ||
                HttpMethod.OPTIONS.equals(method) || HttpMethod.TRACE.equals(method)) {
        ...
        ListenableFuture<ResponseEntity<String>> retryFuture = httpClient.exchange(
            uri, method, entity, String.class);
        retryFuture.addCallback(new ListenableFutureCallback<ResponseEntity<String>>() {
            @Override
            public void onFailure(Throwable throwable) {
                TbMsg next = processException(ctx, msg, throwable);
                ctx.tellFailure(next, throwable);
    ...
    private String buildDigestAuthHeader(String nonce, String realm, String opaque, String uri) {
        String qop = "auth";
